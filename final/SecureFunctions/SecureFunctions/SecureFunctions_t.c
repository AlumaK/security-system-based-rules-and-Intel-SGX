...
static sgx_status_t SGX_CDECL sgx_enclaveStrcpy(void* pms)
{
	...
	enclaveStrcpy(_in_dest, (const char*)_in_src);
err:
	if (_in_dest) {
...
static sgx_status_t SGX_CDECL sgx_enclaveFibonacci(void* pms)
{
	ms_enclaveFibonacci_t* ms = SGX_CAST(ms_enclaveFibonacci_t*, pms);
	sgx_status_t status = SGX_SUCCESS;

	CHECK_REF_POINTER(pms, sizeof(ms_enclaveFibonacci_t));

	ms->ms_retval = enclaveFibonacci(ms->ms_a1);
...
SGX_EXTERNC const struct {
	...
	3,
	{
		...
		{(void*)(uintptr_t)sgx_enclaveFibonacci, 0},
	}
};
